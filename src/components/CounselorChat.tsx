...
import { Send, Heart, Sparkles, Smile, RefreshCw, EyeOff, ShieldAlert, Search, X } from "lucide-react";
...
export default function CounselorChat({ user }: CounselorChatProps) {
  ...
  return (
    <div id="counselor_chat_container" className="flex flex-col h-[650px] bg-white rounded-3xl border border-[#EBE6DD] overflow-hidden shadow-sm">
      ...
      {/* Search Bar Row */}
      <div className="bg-[#FAF8F5] border-b border-[#EBE6DD] px-6 py-2.5 flex items-center justify-between gap-3 text-xs">
        <div className="relative flex-1 flex items-center">
          <span className="absolute left-3 text-[#7A746A]">
            <Search className="w-3.5 h-3.5" />
          </span>
          <input
            id="past_messages_search_input"
            type="text"
            value={searchQuery}
            onChange={(e) => setSearchQuery(e.target.value)}
            placeholder="상담 나눴던 대화의 키워드를 입력하여 찾으실 수 있습니다..."
            className="w-full pl-9 pr-8 py-2 bg-white border border-[#EBE6DD] rounded-xl focus:outline-none focus:ring-1 focus:ring-[#4C6444] focus:border-[#4C6444] text-xs text-stone-800 placeholder-[#ADA497]"
          />
          {searchQuery && (
            <button
              type="button"
        ...
        {searchQuery && (
          <span className="text-[11px] text-[#4C6444] font-bold bg-[#EAF0E6] px-2.5 py-1.5 rounded-xl shrink-0 select-none">
            {filteredMessages.length}개 대화
          </span>
        )}
      </div>
